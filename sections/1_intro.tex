...
\begin{enumerate}
    ...
    opinions are treated vary between contested (controversial) and
    consensual (non-controversial) posts?
...
\begin{enumerate}
    \item \textbf{Minority status becomes consequential only against a consensus.} In consensual posts, minority comments receive substantially lower scores and substantially more replies than majority comments. In contested posts these majority–minority differences are small or absent. The crowd therefore responds not to numerical rarity but to a judgment's position against a clear collective verdict, which is what separates ordinary disagreement from dissent.

    This claim rests on a measurement choice. Existing work on the attention–approval split infers norm deviation from a comment's own low score \cite{davis2021emotional}, which leaves deviance and sanction entangled in a single variable. We instead read a comment's substantive moral judgment against the dominant judgment of its post, so that dissent is defined by content and its treatment by score, replies, and depth. Together with the contested cohort as a control—where the two sides are near-symmetric by construction—this decoupling is what makes the comparison above possible; under a score-based definition of deviance it is unavailable even in principle.
    ...
    \item \textbf{Dissent is directionally asymmetric.} Accusing an OP the crowd absolved is punished more heavily, yet draws roughly 1.7–1.9 times more discussion, than defending an OP the crowd condemned. Accusations against a lenient consensus provoke confrontation; defenses of a condemned OP are more likely to be ignored. Sanction and attention are thus not two readings of a single severity—the moral direction of the challenge moves them apart, connecting the treatment of dissent to the asymmetry between blame and exculpation in moral judgment.
