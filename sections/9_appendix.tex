...
blame in the same direction as \emph{YTA} and \emph{NTA}, we fold them into
those tags, leaving the four that make up the exculpating group $\mathrm{NA}$
and the inculpating group $\mathrm{YA}$ (\S\ref{subsec:dataset}).
...
\begin{table}[t]
  ...
  collapsed into the exculpating group $\mathrm{NA}$ and the inculpating group
  $\mathrm{YA}$, with the hypothetical forms \emph{YWNBTA} and \emph{YWBTA}
  folded into \emph{NTA} and \emph{YTA}; \emph{INFO} states no judgment and is
  discarded. AITAH also accepts H-suffixed
